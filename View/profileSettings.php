...
if(isset($_SESSION['idclient'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(isset($_POST['publish'])){
            ...
            header("Location: profileSettings.php?change=success");
        }
        ...
        if(isset($_POST['delete_account'])){
            ...
            $recupPassword = $bdd->prepare("SELECT client_password FROM client WHERE id = ?");
            $recupPassword->execute([$userId]);
            $passwordBdd = $recupPassword->fetch();

            if(password_verify($password, $passwordBdd['client_password'])){

                deleteUser($bdd,$userId);

                $deleteUser = $bdd->prepare("DELETE FROM client WHERE id = ?");
                $deleteUser->execute([$userId]);

                $_SESSION = array();
                session_destroy();

                header ("Location: deleteAccountConfirm.php");
            } else {
                ...
                <form id="editor-form" method="post" action="" enctype="multipart/form-data">
                    <div class="row">
                        <div class="form-group mb-4 col-md-6">
                            ...
                            <input id="name" class="form-control" name="name" placeholder="Modifier votre nom" type="text" value="<?php echo $nom; ?>" maxlength="64">
                        </div>
                        <div class="form-group mb-4 col-md-6">
                            <label for="firstname" class="col-12">Modifier votre prénom</label>
                            <input id="firstname" class="form-control" name="firstname" placeholder="Modifier votre prénom" type="text" value="<?php echo $prenom; ?>" maxlength="64">
                        </div>
                    </div>
                    <div class="form-group mb-4 col-md-6">
                        <label for="pseudo" class="col-12">Modifier votre pseudo</label>
                        <input id="pseudo" class="form-control" name="pseudo" placeholder="Modifier votre pseudo" type="text" value="<?php echo $pseudo; ?>" maxlength="64">
                    </div>
                    <div class="form-group mb-4 col-md-6">
                        <label for="humeur" class="col-12">Modifier votre humeur</label>
                        <input id="humeur" class="form-control" name="humeur" placeholder="Modifier votre humeur" type="text" value="<?php echo $humeur; ?>" maxlength="45">
                    </div>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="submit" class="btn btn-danger" name="delete_account">Oui, supprimer mon compte</button>
                                </div>
                                </form>
                            </div>
                        </div>
