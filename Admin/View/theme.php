...
if (isset($_SESSION['idclient'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST['newTheme'])){
            $theme = isset($_POST['theme']) ? htmlspecialchars($_POST['theme']) : '';


            $newTheme = $bdd->prepare("INSERT INTO travel_theme (theme_name) VALUES (?)");
            $newTheme->execute([$theme]);

            header("Location: theme.php?add=success");
        }

        if (isset($_POST['delete']) && $_POST['delete'] === 'Oui') {
            $id = $_POST['id'];
            suppr_sql('travel_theme', $id,$bdd);  // Assurez-vous que cette fonction est bien définie
            header("Location: theme.php?delete=success");
            exit;
        }
        ...
        if (isset($_POST['update']) && $_POST['update'] === 'modify') {
            ...
            foreach ($_POST[$id . 'tab'] as $key => $value) {
                modify_sql('travel_theme', $id, $key, $value,$bdd);
            }
            header("Location: theme.php?change=success"); // Redirection pour éviter le rechargement du formulaire
            exit;
        }
...
function get_table($nom_table, $bdd){
    ...
    $query = $bdd->query('SELECT id,theme_name FROM '.$nom_table);
        ...
        <table class="table table-bordered table-custom-alternative-row-color">
            ...
            if (!empty($donnees)) {
                foreach ($donnees as $objet) {
                    ...
                    $id = $objet[$head[0]];
                    $query = $bdd->prepare('SELECT COUNT(*) AS count FROM travel WHERE idtheme = ?');
                    $query->execute([$id]);
            ...
            <button type='button' class='btn btn-primary' onclick='openModifyModal($id)'>Modifier</button>";
                    if ($count == 0) {
                        echo "<button type='button' class='btn btn-danger' onclick='openDeleteModal($id)'>Supprimer</button>";
                    }
                    echo "</td></tr>";
                }
            }
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination justify-content-center">
                                <li class="page-item <?php if($page <= 1) echo 'disabled'; ?>">
                                    <a class="page-link" href="theme.php?taille_tableau=<?php echo $taille; ?>&n_page=<?php echo $page - 1; ?>">Précédent</a>
                                </li>
                                ...
                                <li class="page-item <?php if($page >= $max_page) echo 'disabled'; ?>">
                                    <a class="page-link" href="theme.php?taille_tableau=<?php echo $taille; ?>&n_page=<?php echo $page + 1; ?>">Suivant</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
            ...
            <form action="" method="POST">
                <div class="form-row">
                    <div class="col-12 col-md-6 mb-3">
                        <label for="newTheme">Ajouter un thème</label>
                        <input type="text" class="form-control" id="newTheme" name="theme" placeholder="Saisissez votre theme" required maxlength="128">
                    </div>
                    <div class="col-lg-2 col-md-12 mb-3 align-self-end">
                        <button type="submit" name="newTheme" class="btn btn-primary btn-block">Créer</button>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    ...
    async function openModifyModal(id) {
        try {
            const response = await fetch(`databaseTreatment.php?id=${id}&table=theme`);
